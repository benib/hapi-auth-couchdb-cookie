    ...
    return {
      ...
      authenticate: function(request, reply) {
        ...
        var pluginSettings = request.route.settings.plugins[
          ...
          usernameParam = options.usernameParam,
          passwordParam = options.passwordParam;

        if (pluginSettings && pluginSettings.usernameParam) {
          usernameParam = pluginSettings.usernameParam;
        }
        if (pluginSettings && pluginSettings.passwordParam) {
          passwordParam = pluginSettings.passwordParam;
        }

        nano(options.couchdbUrl).auth(
          request.params[usernameParam],
          request.params[passwordParam],
          function(error, body, headers) {
            if (error || !body.ok) {
